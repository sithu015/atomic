...
async fn fallback_serves_shell_assets_and_never_shadows_explicit_routes() {
    ...
    for path in ["/", "/login", "/account/provider", "/signup"] {
        ...
        let content_type = resp
            ...
            .unwrap_or_default()
            .to_string();
        assert!(content_type.contains("text/html"), "{path} is HTML: {content_type}");
    ...
    let content_type = resp
        ...
        .unwrap_or_default()
        .to_string();
    assert!(content_type.contains("javascript"), "asset is JS: {content_type}");
    let cache = resp
        .headers()
        .get("cache-control")
        .and_then(|v| v.to_str().ok())
        .unwrap_or_default()
        .to_string();
    assert!(cache.contains("immutable"), "hashed asset is immutable: {cache}");
    let body = test::read_body(resp).await;
    ...
    let content_type = resp
        ...
        .unwrap_or_default()
        .to_string();
    assert!(content_type.contains("application/json"), "health stays JSON: {content_type}");
